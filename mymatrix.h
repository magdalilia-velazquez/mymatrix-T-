...
public:
  ...
  void growcols(int r, int C)
  {
    ...
    // Check if the columns are growing or shrinking 
    if (C <= Rows[r].NumCols) // Not growing, shrinking
        return;
    
    // grow row to have C columns
    T* newRow = new T[C];
    
    // copy over existing elements from previous row 
    ...
    // delete old row and replace with new row
    delete[] Rows[r].Cols; 
    Rows[r].Cols = newRow;
    Rows[r].NumCols = C;
  }
